...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[J]ICD 10","[J]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[M]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[A]Lihat Hasil Lab","[A]Lihat Hasil Rad"
        };
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.biaya_pengadaan_dapur,user.rekap_pengadaan_dapur,user.ba_pemusnahan_obat,user.riwayat_pemberian_obat,user.indikator_mutu,user.tarif_mcu,user.seek_lab,user.seek_rad from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
